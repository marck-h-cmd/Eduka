        ...
        <div class="col-12">
            <div class="box_block">
                <button class="estilo-info btn btn-block text-left rounded-0 btn_header header_6" type="button"
                    ...
                    <i class="fas fa-user-shield m-1"></i>&nbsp;Editar Rol: {{ $rol->nombre }}
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="p-2 flex-fill">
                            <p class="mb-0" style="font-size:1.1rem; color:#004a92; font-weight:700;">
                                Modifique los datos del rol según sea necesario. Todos los campos marcados con * son obligatorios.
                            </p>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text"
                                            ...
                                            name="nombre" value="{{ old('nombre', $rol->nombre) }}" required
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('descripcion') is-invalid @enderror" id="descripcion" name="descripcion"
                                            rows="4" style="border-color: #007bff;">{{ old('descripcion', $rol->descripcion) }}</textarea>
                                        @error('descripcion')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
    ...
    $(document).ready(function() {
        ...
        $('#nombre').on('input', function() {
            ...
            if (nombre.length === 0) {
                ...
                // Si el nombre no cambió, mostrar válido
                if (nombre.toLowerCase() === nombreOriginal.toLowerCase()) {
                ...
                success: function(response) {
                    ...
                    if (response.roles && response.roles.data) {
                        existe = response.roles.data.some(function(rol) {
                            return rol.nombre.toLowerCase() === nombre.toLowerCase() && rol.id_rol != {{ $rol->id_rol }};
                        });
                    }
        ...
        // Inicializar validación para valores existentes
        if (nombreOriginal) {
            $('#nombreHelp').removeClass('text-danger text-warning').addClass('text-success').text(
                'Nombre válido');
            $('#nombre').removeClass('is-invalid').addClass('is-valid');
        ...
        if ('{{ $rol->descripcion }}') {
            var descLength = '{{ $rol->descripcion }}'.length;
            $('#descripcionHelp').removeClass('text-danger text-warning').addClass('text-success')
                .text('Descripción válida (' + descLength + '/255 caracteres)');
            $('#descripcion').removeClass('is-invalid').addClass('is-valid');
        }
