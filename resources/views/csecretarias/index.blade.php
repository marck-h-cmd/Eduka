        ...
        <div class="col-12">
            <div class="box_block">
                <button class="estilo-info btn btn-block text-left rounded-0 btn_header header_6" type="button"
                    ...
                    <i class="fas fa-user-tie m-1"></i>&nbsp;Gestión de Secretarias
                ...
                <div class="card-body info">
                    <div class="d-flex ">
                        ...
                        <div class="p-2 flex-fill">
                            <p>
                                En esta sección, podrás gestionar las secretarias del sistema, incluyendo su información personal y laboral.
                            </p>
                            <p>
                                Estimado Usuario: Asegúrate de revisar cuidadosamente los datos antes de guardarlos, ya que esta información será utilizada para la gestión administrativa.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card card-body rounded-0 border-0 pt-0 pb-2" style="background-color: #fcfffc !important">
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-6 d-flex justify-content-md-end justify-content-start estilo-info">
                                <form id="formBuscar" method="GET" action="{{ route('secretarias.index') }}" class="w-100" style="max-width: 100%;">
                                    <div class="input-group">
                                        <input name="buscarpor" id="inputBuscar" class="form-control mt-3" type="search" placeholder="Buscar secretaria" aria-label="Search" autocomplete="off" style="border-color: #007bff;" value="{{ request('buscarpor') }}">
                                ...
                                <tbody>
                                    @forelse($secretarias as $secretaria)
                                        <tr>
                                            ...
                                            <td>{{ $secretaria->persona->dni }}</td>
                                            <td>{{ $secretaria->persona->telefono ?? 'N/A' }}</td>
                                            <td>
                                                <a href="mailto:{{ $secretaria->emailUniversidad }}" class="text-primary">
                                                    <i class="fas fa-envelope"></i> {{ $secretaria->emailUniversidad }}
                                                </a>
                                            </td>
                                            <td>{{ $secretaria->fecha_ingreso ? \Carbon\Carbon::parse($secretaria->fecha_ingreso)->format('d/m/Y') : 'N/A' }}</td>
                                            <td>
                                                <span class="badge badge-{{ $secretaria->estado == 'Activo' ? 'success' : 'danger' }}">
                                                    {{ $secretaria->estado }}
                                                </span>
                                            </td>
                                            <td class="btn-action-group">
                                                <a href="{{ route('secretarias.show', $secretaria) }}" class="btn btn-link btn-sm p-0 btn-ver-secretaria" title="Ver">
                                                    <i class="fas fa-eye" style="color: #17a2b8; font-size: 1.2rem;"></i>
                                                </a>
                                                <a href="{{ route('secretarias.edit', $secretaria) }}" class="btn btn-link btn-sm p-0 btn-editar-secretaria" title="Editar">
                                                    <i class="fas fa-pen" style="color: #007bff; font-size: 1.2rem;"></i>
                                                </a>
                                                <button type="button" class="btn btn-link btn-sm p-0 btn-eliminar-secretaria" title="Eliminar"
                                                        onclick="confirmarEliminar({{ $secretaria->id_secretaria }}, '{{ $secretaria->persona->nombres }} {{ $secretaria->persona->apellidos }}')">
                                                    <i class="fas fa-times" style="color: #dc3545; font-size: 1.3rem;"></i>
                                                </button>
                                                <form id="delete-form-{{ $secretaria->id_secretaria }}" action="{{ route('secretarias.destroy', $secretaria) }}" method="POST" class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="9" class="text-center">No hay secretarias registradas.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
                        <div id="tabla-secretarias">
                            {{ $secretarias->links() }}
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function confirmarEliminar(secretariaId, nombreSecretaria) {
        Swal.fire({
            title: '¿Está seguro?',
            text: `¿Desea cambiar el estado de la secretaria "${nombreSecretaria}" a Inactivo?`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                const form = document.getElementById(`delete-form-${secretariaId}`);
                if (form) {
                    form.submit();
                }
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Loader para ir a create (Nueva Secretaria)
        ...
        // Loader para ver
        document.querySelectorAll('.btn-ver-secretaria').forEach(function(btn) {
        ...
        // Loader para editar
        document.querySelectorAll('.btn-editar-secretaria').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                if (loader) {
                    loader.style.display = 'flex';
