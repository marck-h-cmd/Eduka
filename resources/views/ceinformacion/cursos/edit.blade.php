        ...
        <div class="col-12">
            <div class="box_block">
                <button class="estilo-info btn btn-block text-left rounded-0 btn_header header_6" type="button"
                    ...
                    <i class="fas fa-file-signature m-1"></i>&nbsp;Editar Curso
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="p-2 flex-fill">
                            <p class="mb-0" style="font-size:1.1rem; color:#004a92; font-weight:700;">
                                Modifique los datos del curso según sea necesario. Todos los campos marcados con * son obligatorios.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="collapse show" id="collapseExample0">
                    <div class="card card-body rounded-0 border-0 pt-3 pb-2" style="background-color: #f8fbff !important; border: 1px solid #0A8CB3; margin-top: 18px;">
                        <form method="POST" action="{{ route('registrarcurso.update', $curso->curso_id) }}" id="formCurso">
                            @method('PUT')
                            @csrf
                            <div class="row mb-2">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('grado_id') is-invalid @enderror" id="grado_id" name="grado_id" required>
                                            ...
                                            @foreach($grados as $grado)
                                                <option value="{{ $grado->grado_id }}" {{ $curso->grado_id == $grado->grado_id ? 'selected' : '' }}>{{ $grado->descripcion ?? $grado->nombre }}</option>
                                            @endforeach
                                        </select>
                                        @error('grado_id')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('seccion_id') is-invalid @enderror" id="seccion_id" name="seccion_id" required>
                                            ...
                                            @foreach($secciones as $seccion)
                                                <option value="{{ $seccion->seccion_id }}" {{ $curso->seccion_id == $seccion->seccion_id ? 'selected' : '' }}>{{ $seccion->nombre }}</option>
                                            @endforeach
                                        </select>
                                        @error('seccion_id')
                                            <span class="invalid-feedback" role="alert">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('ano_lectivo_id') is-invalid @enderror" id="ano_lectivo_id" name="ano_lectivo_id" required>
                                            ...
                                            @foreach($aniosLectivos as $anio)
                                                <option value="{{ $anio->ano_lectivo_id }}" {{ $curso->ano_lectivo_id == $anio->ano_lectivo_id ? 'selected' : '' }}>{{ $anio->nombre ?? $anio->descripcion }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="aula_id" name="aula_id" required>
                                            ...
                                            @foreach($aulas as $aula)
                                                <option value="{{ $aula->aula_id }}" {{ $curso->aula_id == $aula->aula_id ? 'selected' : '' }}>{{ $aula->nombre }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row mb-2">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="profesor_principal_id" name="profesor_principal_id" required>
                                            ...
                                            @foreach($profesores as $profesor)
                                                <option value="{{ $profesor->profesor_id ?? $profesor->docente_id }}" {{ $curso->profesor_principal_id == ($profesor->profesor_id ?? $profesor->docente_id) ? 'selected' : '' }}>{{ $profesor->nombres ?? $profesor->nombre }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6 mb-2">
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control @error('cupo_maximo') is-invalid @enderror" id="cupo_maximo" name="cupo_maximo" value="{{ $curso->cupo_maximo }}" required>
                                        @error('cupo_maximo')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
